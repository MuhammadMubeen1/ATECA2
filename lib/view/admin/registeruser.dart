...
class _UserProfileState extends State<Userprofile> {
  ...
  void initState() {
    super.initState();
    // Fetch initial restriction and verification status from Firestore
  ...
  Future<void> updateUserVerification(bool newVerification) async {
    try {
      final QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection('RegisterUsers')
          .where('email', isEqualTo: widget.email)
      ...
      if (querySnapshot.docs.isNotEmpty) {
        final doc = querySnapshot.docs.first;
        await doc.reference.update(
            {'verification': newVerification ? 'verified' : 'unverified'});

        setState(() {
          isVerified = newVerification;
        });

        final message = newVerification
        ...
        print(
            'User verification updated successfully. New status: ${newVerification ? 'verified' : 'unverified'}');

        // Check if the new verification status is true and sign out the user if necessary
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
                backgroundColor: AppColor.btnColor,
      ...
      print('Error updating verification: $error');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          backgroundColor: AppColor.btnColor,
          content: Text('Failed to update verification. Please try again.', style: TextStyle(color: Colors.black)),
        ),
      );
    }
  }
Widget build(BuildContext context) {
  return Scaffold(
    ...
    appBar: AppBar(
      ...
      iconTheme:IconThemeData(color:Color(0xff2476BD) ) ,
